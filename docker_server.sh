...
try {
    ...
    if (!logDir.exists()) {
        logDir.mkdirs();  // Creates directories if they do not exist
    }
    ...
    // Log the event
    String logMessage = timestamp + " - Page accessed by a user.\n";
    outLog.println(logMessage);

    // IMPORTANT: Close the writer to save the changes
    outLog.close();
} catch (IOException e) {
    // Basic error handling - log to console or provide further handling
    e.printStackTrace();
}
%>
